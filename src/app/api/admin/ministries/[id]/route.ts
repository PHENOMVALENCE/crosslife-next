...
export async function PUT(request: NextRequest, { params }: Params) {
  ...
  await execute(
    `UPDATE ministries SET name=?, description=?, image_url=?, leader_name=?, contact_email=?, status=?, display_order=? WHERE id=?`,
    [
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  const auth = await requireAdminApi(CONTENT_ROLES);
  if (isApiError(auth)) return auth;
  await execute('DELETE FROM ministries WHERE id = ?', [Number(await params.then((p) => p.id))]);
  return NextResponse.json({ success: true });
}
